...
def topK(score):
    ...
    batch_size, beam_size, vocab_size = score.size() #2,3,4
    flat_score = score.reshape(batch_size,  beam_size * vocab_size) #6 * 4
    top_score, top_indices = torch.topk(flat_score, k=beam_size, dim=-1)
    top_wordid = top_indices % vocab_size
    top_beamid = top_indices // vocab_size
    top_beamid = top_beamid.reshape(batch_size, beam_size)
    top_wordid = top_wordid.reshape(batch_size, beam_size)
...
def select_hiddens_by_beams(hiddens, beam_id):
    ...
    Example:
        ...
        new_hiddens = torch.FloatTensor([[[0.1627, 0.4812, 0.1167, 0.4318],
                                        ...
                                        [0.9372, 0.4993, 0.5471, 0.9169]]])
    """
    batch_size, beam_size, hidden_size = hiddens.shape[0:3]  # handle test case error
    
    new_hiddens = torch.zeros_like(hiddens)
    for i,_ in enumerate(hiddens):
        for j,_ in enumerate(hiddens[i]):
...
def extract_sequences(top_score, top_wordids, top_beamids):
    ...
    # Reverse the sequence to obtain the correct order of word IDs.
    seq_length = len(top_wordids)
    batch_size, beam_size = top_score.shape
    sequences = []
    sort_score = torch.zeros((batch_size, beam_size), dtype=torch.float32)
    for batch in range(batch_size):
        for beam in range(beam_size):
            seq_wordids = []
            beam_ids = []
            seq_wordids.append(top_wordids[-1][batch][beam])
            beam_ids.append(top_beamids[-1][batch][beam])

            for seq in range(seq_length-2,-1,-1):
                beam_id = beam_ids[-1]
                seq_wordids.insert(0,top_wordids[seq][batch][beam_id])
                beam_ids.append(top_beamids[seq][batch][beam_id])
                
            sequences.append(seq_wordids)
            sort_score[batch, beam] = top_score[batch][beam]
            
    sequences = torch.tensor(sequences)
    sequences = sequences.view([batch_size,beam_size,seq_length])
    top_score, indices = torch.sort(top_score, descending=True)
    new_sequence = torch.zeros_like(sequences)
    for batch in range(batch_size):
            new_sequence[batch] = sequences[batch][indices[batch]]
    return sequences, sort_score
